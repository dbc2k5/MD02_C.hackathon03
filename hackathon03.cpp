...
int main(){
	...
	scanf("%d",&n);
	int des=0,as=0;
	int divisible_des[des],divisible_as[as];
	int divisible[n];
	int count;
	if(n>0&&n<100){
	do{
		...
		switch(choice){
			...
			case 7:
				...
				for(int i=0;i<n;i++){
					if(numbers[i]%5==0){
						numbers[i]=divisible[i];
						count++;
					}
				}
				for(int i=0;i<count;i++){
					if(divisible[i]%2==0){
						divisible[i]=divisible_des[des];
					}else{
						divisible[i]=divisible_as[as];
					}
				}
				printf("%d %d",divisible_des[des],divisible_as[as]);
				break;
			case 8:
				break;
			case 9:
